		...
		$.dialog({
			...
			[@compress single_line = true]
				...
				<table id="moreTable" class="moreTable">
					<tr>
						<th>
							${message("Product.shopCategory")}:
						<\/th>
						<td>
							<select name="shopCategoryId">
								<option value="">${message("admin.common.choose")}<\/option>
								[#list shopCategories as shopCategory]
									<option value="${shopCategory.id}"[#if shopCategory.id == shopCategoryId] selected="selected"[/#if]>
										${shopCategory.name}
									<\/option>
								[/#list]
							<\/select>
	...
	<form id="listForm" action="list.jhtml" method="get">
		...
		<table id="listTable" class="list">
			<tr>
				...
				<th>
					<a href="javascript:;" class="sort" name="shopCategory">${message("Product.shopCategory")}</a>
				</th>
				<th>
