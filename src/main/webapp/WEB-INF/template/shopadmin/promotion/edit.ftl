				...
				<tr>
					...
					<td colspan="2">
						<select id="type" name="type">
							<option value="2" [#if promotion.type==2]selected="selected"[/#if]>${message("Promotion.shopType")}</option>
							<option value="1" [#if promotion.type==1]selected="selected"[/#if]>${message("Promotion.productType")}</option>
						</select>
					</td>
				</tr>
			</table>
			<table id="productTable" class="input" [#if promotion.type==2]style="display:none"[/#if]>
				<tr>
					<th>
